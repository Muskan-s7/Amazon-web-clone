...
function renderProductsGrid() {
  ...
  document.querySelectorAll(".js-add-to-cart").forEach((button) => {
    button.addEventListener("click", () => {
      const productId = button.dataset.productId; //productId is attached with the button

      //selects quantity from select option
      const quantitySelector = document.querySelector(
        `.js-quantity-selector-${productId}`
      );
      const quantity = Number(quantitySelector.value);

      //To show "Added" message, when we click AddToCart button
      ...
      //Removes Added msg after 1sec when we click AddToCart button
      setTimeout(() => {
